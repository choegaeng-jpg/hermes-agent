...
class _FailAfterFirstChatReasoningDeltaStream:
    """Chat-completions stream mock: one reasoning delta then crash."""

    def __iter__(self):
        yield _make_stream_chunk(reasoning_content="hidden reasoning")
        raise RuntimeError("chat stream crashed after reasoning-only partial delivery")
...
class TestStreamingFallback:
    ...
    def test_chat_reasoning_only_delta_without_callback_can_fallback(
        ...
        """Chat stream hidden reasoning deltas must not count as partial delivery."""
        from run_agent import AIAgent
